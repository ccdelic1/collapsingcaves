...
public class CaveInBlockRegistry {
    ...
    static {
        ...
        // Note: Polished Tuff and other tuff variants do not exist until 1.21 and have
        // been removed for this backport.
        ...
        // Note: Sulfur/Cinnabar (Sulfur Caves) blocks do not exist in 1.21.1 and
        // have been removed for this backport.
    ...
    public static void resolve() {
        ...
        for (String id : config.whitelistedBlocks) {
            Block block = resolveConfiguredBlock(id, "whitelistedBlocks");
            if (block != null) {
        ...
        for (String id : config.blacklistedBlocks) {
            Block block = resolveConfiguredBlock(id, "blacklistedBlocks");
            if (block != null) {
    ...
    private static Block resolveConfiguredBlock(String id, String configFieldName) {
        ResourceLocation loc = ResourceLocation.tryParse(id);
        if (loc == null) {
            CollapsingCaves.LOGGER.warn("Ignoring invalid block id \"{}\" in {}: not a valid namespaced identifier.", id, configFieldName);
            return null;
        }
        Block block = BuiltInRegistries.BLOCK.get(loc);
        if (block == Blocks.AIR) {
            CollapsingCaves.LOGGER.warn("Ignoring unknown block id \"{}\" in {}: no such block is registered.", id, configFieldName);
            return null;
        }
        return block;
    }
